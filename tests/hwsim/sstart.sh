...
if test "$LOGBASEDIR" = "$DIR/logs" -a -w "$LOGBASEDIR" ; then
    ...
    #n -sf "$(basename "$LOGDIR")" "$LOGBASEDIR/current"
	echo "$LOGBASEDIR/current"
fi
...
if [ -f /proc/modules ]; then
    ...
    sudo insmod ${TOPDIR}/hwsim/scm80211_hwsim.ko radios=7 mac_log_level=7 dyndbg=+p
    sudo insmod ${TOPDIR}/mac80211/scm80211.ko dyndbg=+p
fi
